...
export function startRequestingServer(
  ...
  // (Discussed with TAs) Moved this route here because it's a special kind of route
  // that returns a response of type video/quicktime instead of application/json
  const manageVideoInstance = new ManageVideoConcept(db);
  const Sessioning = new SessioningConcept(db);

  app.post("/api/ManageVideo/streamVideo", async (c) => {
    try {
      const body = await c.req.json();
      // Get user from session
      const userResult = await Sessioning._getUser({ session: body.session });
      ...
      if ("error" in firstResult) {
        return c.json({ error: firstResult.error }, 401);
      }

      const user = firstResult.user;
      const { video: videoId } = body;
      if (!videoId || !user) {
        return c.json({ error: "Missing videoId or caller" }, 400);
      }
      const result = await manageVideoInstance.streamVideo({
        video: videoId as Video,
        caller: user as User,
      });
      return result;
    } catch (error) {
  ...
  // Seperate upload to use multipart/form-data because
  // JSON body has size limits and is not suitable for file uploads

  // got error: Error processing request: offset is out of bounds
  // when trying to convert files to base64: file: await toBase64(file)
  app.post("/api/ManageVideo/upload", async (c) => {
    try {
      // Parse multipart/form-data instead of JSON
      const formData = await c.req.formData();

      // Extract session from form data
      ...
      const file = formData.get("file"); // This will be a File object
      const videoName = formData.get("videoName") as string;
      const referenceVideoId = formData.get("referenceVideoId") as string;

      // Validate required fields
      if (!videoType || !file || !videoName || !user) {
        return c.json({ error: "Missing parameters for upload" }, 400);
      }

      const result = await manageVideoInstance.upload({
        owner: user as User,
        videoType,
